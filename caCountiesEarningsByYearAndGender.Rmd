...
yearsSeq <- data.frame(AgeYear =c( rep(seq(1900, 2016, by=1), 116))) 

tst <- ListOfFirstNames %>% 
  dplyr::select (name, year, n, sex) %>%
  mutate(decade = year - (year %% 10)) %>% filter(year >= 1900)

ListOfFirstNames %>% ggplot( aes(x= year, y = n)) +
   geom_bar(stat="identity")

merge(yearsSeq,tst ,all=TRUE) %>% distinct() -> mergedData


mergedData <- mergedData %>% mutate(Age = AgeYear - year) %>% filter(Age > -1)

tst %>% ggplot( aes(x= year, y = n)) +
   geom_bar(stat="identity")
...
# join the names to the lifetables

t <- mergedData %>% left_join(lifetables,
                   by = c("decade" = "year",
                          "sex" = "sex"
                          )) %>% distinct() %>% filter(Age  == x)

year2016 <- t %>% filter(AgeYear == 2016)
