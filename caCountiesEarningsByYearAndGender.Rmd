...
This report tries to mimic the report titled [County Pay Practices](https://www.auditor.ca.gov/pdfs/reports/2015-132.pdf), Report 2015-132.  This report was crearted by the California State Auditor and it sheds light in the wage gap between females and male employees working employed by california counties.
...
The county data was processed and the first names extracted.  Some names are used by male and females almost equally e.g. Alexy or Adel. The data from the R package was summarized and names were given a proportion of usage e.g. Alexy was classified as female and as Adel. Some names are almost exclusively used by females e.g. Therese or Annabelle and some are almost exclusively used by males e.g. Rodrigo or Mauricio. The names from the county data was matched with the names list with fender proportion. After the matching processes some names were left unmatched e.g.Mcheko or SuiKwong.

For this report the median was used because it is less prone to be skewed by total pay observations that are too high or too low.


### Data issues

This analysis uses each row as if it was a unique worker but this raises issues. Some workers may be employees in one department with a certain title e.g. Program Analyst and then promote to e.g. Information Systems Analyst.  In this situations the same worker may appear twice in the data for the same year.  In addition, some workers may start working for the county for part of the year due to being new hire or leaving county employment due to retirement, death, or employment in the private sector.

Some salaries include back pay and settlement payments, which make total pay increase and thus skew the data.

Suggestions to mitigate this situations and get a better idea of county pay is to include a unique identifier by worker (it does not have to be the employee number), and include the number of hours a worker was paid in the year. In addition, Kern County data was especially difficult to match.
...
To measure the gender gap, it is useful to know the number of employees who are above and below the median by county. For this the median pay was extracted, rather than the average, by county and by by year. The records where then grouped into above or below the median and a [Chi-squared test](https://en.wikipedia.org/wiki/Chi-squared_test) was complete by county.  For most counties the number of records classified as female are above that what is expected.
...
DescTable <-  summary(Agency ~ Total.Pay...Benefits,
                      data = t, 
                      method="reverse", 
                      test = T, 
                      continuous = 0)
...
```{r plotDensityHistogramFill, fig.width = 6, fig.height = 8,  eval=TRUE, include=TRUE, fig.cap= "Tectonic changes are needed to close the gap between total earnings between males and females. For Ventura County, the highest paid records are female in 2014.", message = FALSE, warning = FALSE, include=TRUE} 
...
```{r whole county vs target area table, results ='asis', message = FALSE, warning = FALSE, eval = FALSE, include=FALSE}
